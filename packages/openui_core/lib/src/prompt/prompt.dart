...
const String _kGrammarPrimer =
    ...
    '\n'
    r'- `$var = @Query(toolName, namedArg: value, ...)` fetches data from '
    'a registered tool and stores it in the state-var. The slot is '
    '`null` until the fetch completes — render a loading state with '
    r'`$var == null ? Spinner() : Table(rows: $var)`. Re-fire with '
    r'`@Run($var)`.'
    '\n'
